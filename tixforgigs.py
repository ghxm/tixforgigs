...
products = [product for product in products if not any([re.search(ex, product['title'], flags=re.IGNORECASE) for ex in args.exclude]) ]
...
if alert_message != '' or args.test:
    ...
    if not args.test:
        ...
        for recipient in log['recipients'].keys():
            if recipient in args.recipients:
